...
const GENERATED_FILE_SUFFIXES = [".generated.ts", ".generated.tsx", ".gen.ts", ".gen.tsx"] as const;
...
function markerReason(
  ...
  for (const marker of markers) {
    if (normalizedComments.includes(marker.toLowerCase())) {
      const origin =
        useDefaultExclusions && DEFAULT_COMMENT_MARKERS.includes(marker as (typeof DEFAULT_COMMENT_MARKERS)[number])
          ? "default"
          : "user";
...
function readGlobToken(pattern: string, index: number): GlobToken {
  ...
  if (character === undefined) {
    throw new Error("Invalid exclude glob");
  }
  if (character === "*") {
    return pattern[index + 1] === "*" ? { length: 2, regexSource: ".*" } : { length: 1, regexSource: "[^/]*" };
  }
  if (character === "?") {
